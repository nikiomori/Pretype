...
enum BrandMark {
    ...
    static func statusItemImage(_ state: State = .ready, height: CGFloat = 13) -> NSImage {
        ...
        let image = NSImage(size: size, flipped: true) { rect in
            ...
            case .ready, .disabled: // I-beam text cursor
                ...
                caret.stroke()
            case .preparing: // download arrow (loading / fetching model)
                caret.move(to: NSPoint(x: 695, y: 372)); caret.line(to: NSPoint(x: 695, y: 600))
                caret.move(to: NSPoint(x: 645, y: 556))
                caret.line(to: NSPoint(x: 695, y: 614))
                caret.line(to: NSPoint(x: 745, y: 556))
                caret.stroke()
